...
class ogm_continous_S_CSM:
    ...
    def construct_map(self, pose, scan):
        ...
        self.map['variance'] = self.prior**2*np.ones((t.shape[0],1))          # size should be (number of data) x (1)
    ...
    def continuous_S_CSM(self, z, i, k):
        ...
        if d1<self.l:
            self.map['alpha'][i, int(z[idx, 2]-1)] += self.sigma*((1/3)*(2+np.cos(2*np.pi*(d1/self.l)))*(1-(d1/self.l))+(1/(2*np.pi))*np.sin(2*np.pi*d1/self.l))
        # else:
        ...
        for r in samples:
            ...
            if d2<self.l:
                self.map['alpha'][i, self.num_classes] += self.sigma*((1/3)*(2+np.cos(2*np.pi*(d2/self.l)))*(1-(d2/self.l))+(1/(2*np.pi))*np.sin(2*np.pi*d2/self.l))

        # m = self.map['occMap'].data[i, :]
        # if (self.m_i['range'] > min(self.z_max, z[idx, 0] + self.w_obstacle / 2)) or (bearing_min > self.w_beam / 2):
        #     pass
        # elif (z[idx, 0] < self.z_max) and (np.abs(self.m_i['range'] - z[idx, 0]) < self.w_obstacle / 2):
        #     # update occupied grid
        #     d1 = np.sqrt((m[0]-global_x)**2+(m[1]-global_y)**2)
        #     self.map['alpha'][i, int(z[idx, 2]-1)] += self.sigma*((1/3)*(2+np.cos(2*np.pi*(d1/self.l)))*(1-(d1/self.l))+(1/(2*np.pi))*np.sin(2*np.pi*d1/self.l))
        # elif (self.m_i['range'] < z[idx, 0]) and (z[idx, 0] < self.z_max):
        #     # update free grid
        #     d1 = np.sqrt((m[0]-global_x)**2+(m[1]-global_y)**2)
        #     self.map['alpha'][i, self.num_classes] += self.sigma*((1/3)*(2+np.cos(2*np.pi*(d1/self.l)))*(1-(d1/self.l))+(1/(2*np.pi))*np.sin(2*np.pi*d1/self.l))
    ...
    def build_ogm(self):
        ...
        for i in tqdm(range(self.map['size'])):
            ...
            self.map['alpha'] = self.prior_alpha*np.ones((self.size,self.num_classes+1))
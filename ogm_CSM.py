...
class ogm_CSM:
    ...
    def counting_sensor_model(self, z, i):
        ...
        if self.m_i['range']>min(self.z_max,z[k,0]+(self.w_obstacle/2)) or bearing_min>self.w_beam/2:
            pass
        elif z[k,0]<self.z_max and np.abs(r-z[k,0])<self.w_obstacle/2:
            self.map['alpha'] += 1
        elif r<z[k,0] and z[k,0]<self.z_max:
            self.map['beta'] += 1
    ...
    def build_ogm(self):
        ...
        for i in tqdm(range(self.map['size'])):
            ...
            # -----------------------------------------------
            self.map['mean'][i]=self.map['alpha']/(self.map['alpha']+self.map['beta'])
            self.map['variance'][i] =self.map['alpha']*self.map['beta']/((self.map['alpha']+self.map['beta'])**2*(self.map['alpha']+self.map['beta']+1))
